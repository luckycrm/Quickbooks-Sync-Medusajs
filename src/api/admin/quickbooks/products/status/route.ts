...
const getAttachableEntityIds = (attachable: Record<string, unknown>) => {
  const refs = Array.isArray(attachable.AttachableRef)
...
export async function GET(
  ...
  const productModuleService = req.scope.resolve(
    "product",
  ) as unknown as ProductModuleService;

  const [medusaProducts, medusaCount] =
  ...
  const medusaRowsBase = medusaProducts.map((product) => {
    ...
    return {
      ...
      medusaTags: uniqueStrings(
        tags.map(
          (tag) =>
            asString(asRecord(tag)?.value) || asString(asRecord(tag)?.name),
        ),
      ),
  ...
  if (!config.configured) {
    const rows: UnifiedProductRow[] = medusaRowsBase.map((entry) => {
      const variants: ProductVariantSummary[] = entry.variants.map(
        (variant) => ({
    ...
    return res.status(200).json({
      ...
      summary: {
        ...
        quickbooks_items: 0,
        matched_variants: 0,
        missing_variants: rows.reduce(
          (sum, row) => sum + row.unmatched_variant_count,
          0,
        ),
  ...
  try {
    ...
    for (const attachable of quickbooksAttachables) {
      ...
      for (const entityId of getAttachableEntityIds(attachable)) {
        quickbooksImageCounts.set(
          entityId,
          (quickbooksImageCounts.get(entityId) || 0) + 1,
        );
      }
    }
    ...
    const rows: UnifiedProductRow[] = medusaRowsBase.map((entry) => {
      const variants: ProductVariantSummary[] = entry.variants.map(
        (variant) => {
          ...
          const quickbooksItem = sku
            ? quickbooksItemsBySku.get(sku) || null
            : null;
      ...
      const matchedVariants = variants.filter(
        (variant) => !!variant.quickbooks_item_id,
      );
      ...
      const firstQuickbooksItem =
        matchedVariants.length > 0 && matchedVariants[0].quickbooks_item_id
          ? quickbooksItems.find(
              (item) =>
                asString(item.Id) === matchedVariants[0].quickbooks_item_id,
            ) || null
          : null;
      ...
      return {
        ...
        quickbooks_updated_at: asString(
          asRecord(firstQuickbooksItem?.MetaData)?.LastUpdatedTime,
        ),
      };
    });
      ...
      .map((item) => {
        ...
        return {
          ...
          availability: [
            ...
            quickbooksImageCount > 0
              ? "Has QuickBooks Images"
              : "No QuickBooks Images",
          ],
          product_tags: [],
          sales_channels: [],
          image_count: 0,
          ...
          quickbooks_active: asBoolean(item.Active),
          quickbooks_updated_at: asString(
            asRecord(item.MetaData)?.LastUpdatedTime,
          ),
        };
      });

    const combinedRows = [...rows, ...quickbooksOnlyRows];
    const matchedVariants = rows.reduce(
      (sum, row) => sum + row.matched_variant_count,
      0,
    );
    const missingVariants = rows.reduce(
      (sum, row) => sum + row.unmatched_variant_count,
      0,
    );
    ...
    return res.status(200).json({
      ...
      quickbooks: {
        error:
          e instanceof Error
            ? e.message
            : "Unable to load QuickBooks products.",
      },
    });
  }
}
